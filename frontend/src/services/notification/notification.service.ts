...
export class NotificationService {
  ...
  static async notifyNewMessage(message: { id: string; content: string; conversation_id: string; direction: 'inbound' | 'outbound'; created_at: string }): Promise<void> {
    try {
      if (!this.swRegistration) {
        console.warn('[NOTIF DEBUG] Service Worker non enregistré');
        return;
      ...
      await this.swRegistration.showNotification('Nouveau message', {
      ...
      console.error('[NOTIF DEBUG] Erreur lors de l\'envoi de la notification:', error);
      throw error;
    }
  }
